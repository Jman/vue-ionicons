<template>
  <div class="ion power-icon" name="power-icon" :aria-labelledby="iconTitle">
    <svg width="1em" height="1em" viewBox="0 0 512 512"><path d="M256 256c-17.7 0-32-14.3-32-32V64c0-17.7 14.3-32 32-32s32 14.3 32 32v160c0 17.7-14.3 32-32 32z"/><path d="M379 68.8c-5-3-10.8-4.8-17-4.8-17.7 0-32 14.3-32 32 0 6.2 1.8 12 4.8 16.9 2 3.2 4.6 6.1 7.6 8.4 1.2.9 2.4 1.7 3.7 2.5 8.1 5.6 15.8 11.9 23 19.1C399.4 173.1 416 213.3 416 256c0 42.7-16.6 82.9-46.9 113.1C338.9 399.4 298.7 416 256 416c-42.7 0-82.9-16.6-113.1-46.9C112.6 338.9 96 298.7 96 256c0-42.7 16.6-82.9 46.9-113.1 7.1-7.1 14.8-13.5 22.9-19 1.4-.8 2.6-1.6 3.9-2.6 3-2.3 5.5-5.1 7.5-8.3 3.1-4.9 4.8-10.7 4.8-16.9 0-17.7-14.3-32-32-32-6.2 0-12 1.8-16.9 4.8l-.1-.1C72.2 108.8 32 177.7 32 256c0 123.7 100.3 224 224 224s224-100.3 224-224c0-78.3-40.2-147.2-101-187.2z"/></svg>
  </div>
</template>

<script>
  export default {
    name: "power-icon",
    data () {
      let iconTitle = this.title ? this.title : "Power Icon"
      return {
        iconTitle
      }
    },
    props: {
      title: {
        type: String,
        default: ""
      }
    }
  }
</script>
